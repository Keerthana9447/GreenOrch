...
function DashboardPage({ toast }) {
  ...
  return (
    <div>
      ...
      {/* World map last — visual, not critical */}
...
function SimulationPage({ toast }) {
  ...
  return (
    <div>
      {/* ── SECTION 1: ABOVE THE FOLD — Result First ────────────────── */}
      {result ? (
        <div style={{ animation:"fadeIn 0.5s ease" }}>
          {/* BIG IMPACT HEADLINE */}
          <div style={{ background:`linear-gradient(135deg,${C.green}20,${C.teal}10)`, border:`1px solid ${C.green}45`, borderRadius:16, padding:"22px 28px", marginBottom:20, boxShadow:`0 4px 32px ${C.green}15` }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:16 }}>
              <div>
                <div style={{ color:C.green, fontSize:32, fontWeight:900, lineHeight:1.1, marginBottom:8 }}>
                  🚀 {result.selected_region?.carbon_reduction_pct}% CO₂ Reduction Achieved
                </div>
                <div style={{ color:C.text, fontSize:18, fontWeight:700 }}>
                  ✅ Scheduled to&nbsp;
                  <span style={{ color:C.green }}>{result.selected_region?.region_name || result.selected_region?.name}</span>
                </div>
                <div style={{ color:C.muted, fontSize:13, marginTop:6 }}>
                  {result.decision?.explanation?.reasons?.[0] || "Lowest carbon intensity region selected"}
                  &nbsp;·&nbsp;
                  Strategy: <span style={{ color:C.teal, fontWeight:700, textTransform:"capitalize" }}>{result.decision?.strategy}</span>
                </div>
              </div>
              <div style={{ display:"flex", flexDirection:"column", gap:8, alignItems:"flex-end" }}>
                <GreenBadge pct={result.selected_region?.carbon_reduction_pct} size="lg"/>
                <EsgBadge grade={result.decision?.esg_grade} size="lg"/>
                <div style={{ color:C.muted, fontSize:10, fontFamily:"monospace" }}>ID: {result.workload_id}</div>
              </div>
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginTop:20 }}>
              {[
                { icon:"🌱", label:"CO₂ Saved",    value:`${result.impact?.carbon_saved_kg} kg`,   color:C.green,  sub:"vs traditional baseline" },
                { icon:"⚡", label:"Energy Saved",  value:`${result.impact?.energy_saved_kwh} kWh`, color:C.teal,   sub:"reduced power consumption" },
                { icon:"🌳", label:"Trees Equiv.",  value:`${result.impact?.trees_equivalent}`,     color:C.green,  sub:"trees planted equivalent" },
              ].map(({ icon, label, value, color, sub }) => (
                <div key={label} style={{ background:"rgba(0,0,0,0.25)", borderRadius:12, padding:"16px 20px", textAlign:"center", border:`1px solid ${color}30` }}>
                  <div style={{ fontSize:24, marginBottom:6 }}>{icon}</div>
                  <div style={{ color, fontSize:22, fontWeight:900, fontFamily:"monospace" }}>{value}</div>
                  <div style={{ color:C.text, fontSize:12, fontWeight:700, marginTop:2 }}>{label}</div>
          ...
          {/* BEFORE vs AFTER — compact inline comparison */}
          <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr", gap:12, marginBottom:20, alignItems:"center" }}>
            <div style={{ background:`${C.red}12`, border:`1px solid ${C.red}35`, borderRadius:12, padding:"16px 20px" }}>
              <div style={{ color:C.red, fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>❌ BEFORE — Traditional</div>
              <div style={{ color:C.text, fontSize:13, fontWeight:600, marginBottom:10 }}>{result.before_after?.before?.region}</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                {[
                  ["CO₂", `${result.before_after?.before?.carbon_kg} kg`, C.red],
                  ["Energy", `${result.before_after?.before?.energy_kwh} kWh`, C.orange],
                  ["Cost", `$${result.before_after?.before?.cost_usd}`, C.amber],
                  ["ESG", `${result.before_after?.before?.esg_score}/100`, C.red],
                ].map(([l,v,c]) => (
                  <div key={l} style={{ background:"#ffffff06", borderRadius:8, padding:"8px 12px" }}>
                    <div style={{ color:C.muted, fontSize:9 }}>{l}</div>
                    <div style={{ color:c, fontWeight:800, fontFamily:"monospace", fontSize:13 }}>{v}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:6 }}>
              <div style={{ color:C.green, fontSize:28, fontWeight:900 }}>→</div>
              <div style={{ background:`${C.green}20`, border:`1px solid ${C.green}50`, borderRadius:10, padding:"8px 14px", textAlign:"center" }}>
                <div style={{ color:C.green, fontSize:20, fontWeight:900 }}>{result.before_after?.efficiency_gains?.carbon_reduction_pct}%</div>
                <div style={{ color:C.muted, fontSize:8, textTransform:"uppercase" }}>CO₂ less</div>
              </div>
            </div>

            <div style={{ background:`${C.green}12`, border:`1px solid ${C.green}35`, borderRadius:12, padding:"16px 20px" }}>
              <div style={{ color:C.green, fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>✅ AFTER — DhruvCloud Nexus</div>
              <div style={{ color:C.text, fontSize:13, fontWeight:600, marginBottom:10 }}>{result.before_after?.after?.region}</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                {[
                  ["CO₂", `${result.before_after?.after?.carbon_kg} kg`, C.green],
                  ["Energy", `${result.before_after?.after?.energy_kwh} kWh`, C.teal],
                  ["Cost", `$${result.before_after?.after?.cost_usd}`, C.amber],
                  ["ESG", `${result.before_after?.after?.esg_score}/100`, C.green],
                ].map(([l,v,c]) => (
                  <div key={l} style={{ background:"#ffffff06", borderRadius:8, padding:"8px 12px" }}>
                    <div style={{ color:C.muted, fontSize:9 }}>{l}</div>
                    <div style={{ color:c, fontWeight:800, fontFamily:"monospace", fontSize:13 }}>{v}</div>
          ...
          {/* ── SECTION 2: KPI Cards + Region Table ─────────────────── */}
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
            {[
              { label:"Best Region",    value:(result.selected_region?.region_name||"").split("(")[0].trim(), accent:C.green, icon:"🏆" },
              { label:"Latency",        value:`${result.selected_region?.latency_ms}ms`,                      accent:C.blue,  icon:"⚡" },
              { label:"Workload Cost",  value:`$${result.selected_region?.cost_usd}`,                         accent:C.amber, icon:"💰" },
              { label:"ESG Grade",      value:result.decision?.esg_grade,                                     accent:C.purple,icon:"📋" },
            ].map(({ label, value, accent, icon }) => (
              <div key={label} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"16px 20px", position:"relative", overflow:"hidden", boxShadow:`0 2px 12px rgba(0,0,0,0.2)` }}>
                <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${accent},transparent)` }}/>
                <div style={{ color:C.muted, fontSize:10, fontWeight:700, letterSpacing:1.2, textTransform:"uppercase", marginBottom:6 }}>{icon} {label}</div>
                <div style={{ color:accent, fontSize:20, fontWeight:800, fontFamily:"monospace" }}>{value}</div>
              </div>
            ))}
          </div>
          ...
          {/* ── SECTION 3: Charts + Map (low priority) ───────────────── */}
          {result.before_after && <BeforeAfterPanel ba={result.before_after}/>}
          <WorldMap regions={RMOCK} selectedId={result.selected_region?.region_id}/>

          {/* ML info — minimal, at bottom */}
          {result.ml && (
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"10px 16px", display:"flex", gap:16, alignItems:"center", flexWrap:"wrap", opacity:0.8 }}>
              ...
              <span style={{ color:C.muted, fontSize:11 }}>{result.ml.model}</span>
        ...
        <div style={{ animation:"fadeIn 0.5s ease" }}>
          <div style={{ background:`linear-gradient(135deg,${C.green}18,${C.teal}08)`, border:`1px solid ${C.green}40`, borderRadius:16, padding:"20px 26px", marginBottom:20 }}>
            <div style={{ color:C.green, fontSize:28, fontWeight:900, marginBottom:6 }}>
              🚀 {simResult.summary?.carbon_reduction_pct}% CO₂ Reduction — Batch of {simResult.workloads_processed}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginTop:16 }}>
              {[
                { l:"CO₂ Saved",    v:`${simResult.summary?.carbon_saved_kg} kg`, c:C.green },
                { l:"Cost Saved",   v:`$${simResult.summary?.total_cost_saved_usd}`, c:C.amber },
                { l:"Avg ESG Gain", v:`+${simResult.summary?.avg_esg_gain}pts`, c:C.purple },
                { l:"Trees 🌳",     v:`${simResult.summary?.equivalents?.trees_planted}`, c:C.green },
              ].map(({ l, v, c }) => (
                <div key={l} style={{ background:"rgba(0,0,0,0.25)", borderRadius:10, padding:"12px 16px", textAlign:"center" }}>
                  <div style={{ color:C.muted, fontSize:10, marginBottom:4 }}>{l}</div>
                  <div style={{ color:c, fontWeight:800, fontFamily:"monospace", fontSize:16 }}>{v}</div>
                </div>
              ))}
            </div>
          </div>
          <div style={{ maxHeight:320, overflowY:"auto", background:C.card, borderRadius:12, padding:16 }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
              <thead><tr>{["ID","Type","CPU","Before CO₂","After CO₂","Region","Reduction","Cost Saved"].map(h => <th key={h} style={{ color:C.muted, textAlign:"left", padding:"5px 8px", borderBottom:`1px solid ${C.border}`, fontSize:9, fontWeight:600 }}>{h}</th>)}</tr></thead>
        ...
        /* ── EMPTY STATE ──────────────────────────────────────────── */
        <div style={{ background:C.card, border:`1px dashed ${C.border}`, borderRadius:16, padding:"48px 32px", textAlign:"center", marginBottom:20 }}>
          <div style={{ fontSize:52, marginBottom:14 }}>☸️</div>
          <div style={{ color:C.text, fontSize:18, fontWeight:700, marginBottom:8 }}>Carbon-Aware K8s Scheduler</div>
          <div style={{ color:C.muted, fontSize:13, marginBottom:20 }}>Submit a workload to see how DhruvCloud Nexus reduces your cloud carbon footprint</div>
          <div style={{ display:"flex", justifyContent:"center", gap:16, flexWrap:"wrap" }}>
            {[["🌿 Carbon First","Minimize CO₂"],["⚖️ Balanced","CO₂ + Cost"],["💰 Cost First","Minimize spend"]].map(([label,sub]) => (
              <div key={label} style={{ background:C.surface, borderRadius:10, padding:"12px 20px", border:`1px solid ${C.border}` }}>
                <div style={{ color:C.text, fontWeight:700, fontSize:12 }}>{label}</div>
                <div style={{ color:C.muted, fontSize:10, marginTop:2 }}>{sub}</div>
              </div>
            ))}
          </div>
      ...
      {/* ── FORM + BATCH — always visible below result ──────────────── */}
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
          ...
          {step > 0 && (
            <div style={{ background:`${C.teal}12`, border:`1px solid ${C.teal}35`, borderRadius:8, padding:"10px 14px", marginBottom:14 }}>
              <div style={{ color:C.teal, fontSize:11, fontWeight:700, marginBottom:6, display:"flex", alignItems:"center", gap:6 }}>
                <Spinner size={12} color={C.teal}/> ⚡ Optimizing across global regions...
              </div>
              {STEPS.map((msg, i) => (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:11 }}>
            ...
            <button onClick={handleSubmit} disabled={loading || step > 0}
              style={{ background:loading||step>0 ? C.border : `linear-gradient(135deg,${C.green},${C.teal})`, border:"none", borderRadius:8, color:"#000", fontWeight:800, fontSize:14, padding:"13px", cursor:loading||step>0?"default":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, fontFamily:"monospace", boxShadow:loading||step>0?"none":`0 4px 16px ${C.green}40` }}>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20, display:"flex", flexDirection:"column" }}>
          <SH title="🚀 Batch Simulation" sub="Run 20 historical workloads · see aggregate impact"/>
          <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", gap:14 }}>
            <div style={{ background:C.surface, borderRadius:10, padding:"14px 18px" }}>
              <div style={{ color:C.text, fontSize:13, fontWeight:700, marginBottom:6 }}>What this shows:</div>
              {["20 real workloads from Google Cluster Trace","Before vs After CO₂ for each","Total carbon saved across batch","Cost savings + ESG gain per workload"].map((t,i) => (
                <div key={i} style={{ color:C.muted, fontSize:11, marginBottom:4, display:"flex", gap:8 }}>
                  <span style={{ color:C.green }}>✓</span>{t}
                </div>
              ))}
            </div>
            <button onClick={handleSim} disabled={simLoading}
              style={{ background:simLoading?C.border:`linear-gradient(135deg,${C.blue},${C.purple})`, border:"none", borderRadius:8, color:"#fff", fontWeight:700, fontSize:13, padding:"13px", cursor:simLoading?"default":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, fontFamily:"monospace" }}>
...
export default function App() {
  ...
  const nav = [
    { id:"simulation", label:"🚀 Scheduler",      icon:"☸️" },
